...
if st.session_state.user_name == "vishakha":
    ...
    template = (
        ...
        1. After gathering Make, Model, and New/Used info from the customer, provide car details only when the model and new or used car details are available.
        2. For appointments, check availability using our appointment schedule data, stored in the `df` dataframe. If no specific details are provided in the inquiry, engage with the customer to ascertain their preferences.
        3. Before scheduling, inquire about car trade-in. Ask for name, contact number, and email separately.
        4. If they ask for rescheduling, then provide the answer by checking the availability and also provide a link:
